...
cd geo-attendance-backend/backend
...
start-up inside the `backend/` directory.
...
4. Deploy!  Render will run `gunicorn app:app` inside the `backend/` directory
   and expose the service on a public URL.
